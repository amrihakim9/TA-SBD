...
if(isset($_GET['Id'])){
	...
	if(mysqli_num_rows($cek) > 0){
		...
		$del = mysqli_query($koneksi, "DELETE FROM pembeli WHERE Id='$Id'") or die(mysqli_error($koneksi));
		if($del){
		...
		echo '<script>alert("ID tidak ditemukan di database."); document.location="index.php?page=tampil_dp";</script>';
	}
}else{
	echo '<script>alert("ID tidak ditemukan di database."); document.location="index.php?page=tampil_dp";</script>';
}
